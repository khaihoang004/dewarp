...
from src.utils.post_processing import flatten_background_color

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = FastAPI(title="DocEnhance API", version="1.0.0")
...
def full_image_inference(model, x, multiple=8):
    """
    Chạy suy luận trên toàn bộ ảnh.
    Pad để chiều cao/rộng chia hết cho 8.
    """
    _, _, h, w = x.shape
...
def sliding_window_inference(model, x, patch_size=512, overlap=128):
    """
    Cắt ảnh lớn thành patch, chạy model từng patch rồi blend lại.
    """
    _, _, H, W = x.shape
...
def preprocess_for_inference(x, ori_h, ori_w, max_size=2048):
    """
    Resize ảnh lớn để tránh OOM.
    Trả về:
      x_processed, is_resized
    """
    is_resized = False
