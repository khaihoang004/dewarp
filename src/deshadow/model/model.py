...
class LoopRepDocEnhanceNet(nn.Module):
    def __init__(self, base_dim=32, max_loops=4, num_heads=4, enc_blocks=[1, 1, 2], dec_blocks=[1, 1, 2], deploy=False):
        ...
        assert (base_dim * 4) % num_heads == 0, f"base_dim*4 ({base_dim*4}) phải chia hết cho num_heads ({num_heads})"
    ...
    def forward(self, x, halt_threshold=0.80, calc_loss=False, return_steps=False):
        ...
        """
        x_ori = x.clone() if (self.training or calc_loss) else x
        ...
        b_out = self.bottleneck(e3, halt_threshold=halt_threshold, calc_loss=calc_loss)
        final_feat = b_out["final_state"]
        
        output = self._decode(final_feat, skip1, skip2, skip3, x_ori)
    ...
    def fuse_entire_model(self):
        ...
        print(f">>> ĐÃ FUSE THÀNH CÔNG: Mạng đã chuyển sang chế độ deploy.")